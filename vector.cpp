...
Vec2f::operator sf::Vector2f() const { return sf::Vector2f(x, y); }

Vec2f Vec2f::operator+(const Vec2f &other) const {
  return Vec2f(x + other.x, y + other.y);
}
Vec2f Vec2f::operator-() const { return Vec2f(-x, -y); }
Vec2f Vec2f::operator-(const Vec2f &other) const { return *this + -other; }
...
bool Vec2f::operator!=(const Vec2f &other) const { return !(*this == other); }
float Vec2f::squareSize() const { return (x * x + y * y); }
float Vec2f::size() const { return std::sqrt(squareSize()); }